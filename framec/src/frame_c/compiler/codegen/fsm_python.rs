...
impl<'a> Generator<'a> {
    ...
    /// (`[0-9A-Za-z_]`)? Out-of-range positions are non-word, so a boundary
    /// at the input edge resolves correctly. Used by the `\b`/`\B` guards.
    fn emit_word_boundary_helper(&self, out: &mut String) {
        writeln!(
            out,
            "    def _iswordat(self, p):\n\
             \x20       if p < 0 or p >= len(self.{inp}):\n\
             \x20           return False\n\
             \x20       b = ord(self.{inp}[p])\n\
             \x20       return (48 <= b <= 57) or (65 <= b <= 90) or (97 <= b <= 122) or b == 95\n",
    ...
    fn emit_pike_matcher(&self, out: &mut String) {
        writeln!(
            ...
            "    def _pike_add(self, ops, lst, seen, pc):\n\
             ...
             \x20       while True:\n\
             \x20           nlist = []\n\
             \x20           nseen = [False] * ninst\n\
             \x20           for pc in clist:\n\
             \x20               op = ops[pc * 4]\n\
             \x20               if op == 0:\n\
             \x20                   if pos < n:\n\
             \x20                       v = ord(self.{inp}[pos])\n\
             \x20                       rs = ops[pc * 4 + 1]\n\
             \x20                       rc = ops[pc * 4 + 2]\n\
            ...
            match first_stage {
                Some(fs) => {
                    ...
                    *sid += 1;
                    ...
                    writeln!(out, "        _r = {}", stage_call(self, sel, my_sid)).ok();
    ...
    fn emit_anchor_guards(&self, out: &mut String, sid: usize, ind: &str) {
        ...
        // `_iswordat`. `_r >= 0` guards the end check so a prior miss stays a
        // miss. A boundary is present iff the two sides differ in word-ness.
        ...
        if let Some(kind) = dfa.end_boundary {
            ...
            writeln!(
                out,
                "{ind}if _r >= 0 and self._iswordat(_r - 1) {op} self._iswordat(_r):\n{ind}    _r = -1"
            )
            .ok();
        }
    }
        ...
        match el {
            MatchElement::Stage(stage) => {
                ...
                *sid += 1;
                ...
                writeln!(out, "{}_r = {}", ind, stage_call(self, stage, my_sid)).ok();
...
mod tests {
    ...
    #[test]
    fn fsm_lazy_quantifier() {
        let src = "@@fsm M(text: bytes) : bytes = \"\" { /.*?,/ @@:matched }";
        let Some(v) = eval_py(src, "ab,cd,ef", &["m.return_value"], "lza") else {
            return;
        };
        assert_eq!(v, vec!["'ab,'"]);
        let mixed = "@@fsm M(text: bytes) : int = 0 { /a*?b+/ @@:cursor }";
        assert_eq!(
            eval_py(mixed, "aabbb", &["m.return_value"], "lzb").unwrap(),
            vec!["5"]
        );
    }
    ...
    #[test]
    fn fsm_word_boundary() {
        let src = "@@fsm M(text: bytes) : bool = false { /\\bcat\\b/ true }";
        let Some(cat) = run(src, "cat", "wba") else {
            return;
        };
        assert_eq!(cat.accepted, "True");
        assert_eq!(run(src, "cats", "wbb").unwrap().accepted, "False");
