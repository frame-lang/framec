...
struct StageDfa {
    ...
    /// `Some` when the stage's regex has a lazy quantifier (§11.1): a Pike VM
    /// program (`pike_match`) replaces the DFA, for leftmost-first semantics.
...
impl<'a> Generator<'a> {
    ...
    fn emit(&self) -> Result<String, String> {
        ...
        self.emit_fsm_input_trait(&mut out);
    ...
    /// The element type the recognizer reads from its input source: a token
    /// kind name (`String`) or a byte/char code point (`char`).
    fn input_elem(&self) -> &'static str {
        match self.alphabet {
    ...
    /// RFC-0042.1 — the per-fsm input-source trait. The recognizer is generic
    /// over `I: <Name>Input`, so the host owns the storage strategy: an owned
    /// buffer (`Vec<char>`/`Vec<String>` — also the back-compat `new` path), a
    /// borrowed slice (`&[char]`/`&[u8]`/`&[String]` — zero-copy), or a callback
    /// (`<Name>Fn(closure, len)`). All reads go through `fsm_get`/`fsm_len`.
    fn emit_fsm_input_trait(&self, out: &mut String) {
        let name = &self.decl.name;
        let elem = self.input_elem();
        let get_owned = if elem == "char" {
            "self[i]"
        } else {
            "self[i].clone()"
        };
        ...
        writeln!(
            out,
            "impl {name}Input for {owned} {{ fn fsm_get(&self, i: usize) -> {elem} {{ {get_owned} }} fn fsm_len(&self) -> usize {{ self.len() }} }}",
            owned = self.input_type()
        )
        .ok();
        // Borrowed slice of the element type.
        writeln!(
            out,
        ...
        // Host callback: `<Name>Fn(|i| elem, len)`.
        writeln!(
            out,
    ...
    fn matched_slice(&self) -> String {
        ...
        format!(
            "(self.cursor..(_r as usize)).map(|__k| self.{}.fsm_get(__k)).collect()",
            inp
        )
    }
    ...
    fn element_read(&self) -> String {
        let inp = &self.decl.params[0].name;
        match self.alphabet {
            Alphabet::Token => format!("self.tok_id(&self.{}.fsm_get(pos))", inp),
            _ => format!("self.{}.fsm_get(pos) as i64", inp),
        }
    }

    fn emit_struct(&self, out: &mut String) {
        writeln!(out, "pub struct {n}<I: {n}Input> {{", n = self.decl.name).ok();
        ...
        for (i, p) in self.decl.params.iter().enumerate() {
            if i == 0 {
                // The input parameter is the generic input source (RFC-0042.1).
                writeln!(out, "    pub {}: I,", p.name).ok();
            } else {
                writeln!(
                    out,
                    "    pub {}: {},",
        ...
        // Pinned to the owned input form — the inner is driven over an owned
        // window materialized through the outer's accessor (RFC-0042.1), which
        // avoids a self-referential borrow of `self.{input}`.
        for (f, inner) in self.mode_c_inst_fields() {
            writeln!(out, "    {}: Option<{}<{}>>,", f, inner, self.input_type()).ok();
        }
        out.push_str("}\n\n");
    }
    ...
    /// `over(input, …)` — build the recognizer over an input source WITHOUT
    /// running (RFC-0042.1). Drive it with `scan_at`. `new` is `over` + `run`.
    fn emit_over(&self, out: &mut String) {
        writeln!(out, "    pub fn over({}) -> Self {{", self.ctor_sig()).ok();
        out.push_str("        Self {\n");
    ...
    /// source; reusable across many offsets (RFC-0042.1). Resets all per-scan
    /// state to its `over` value, then runs. Returns `accepted`; the host reads
    /// `self.cursor` / `self.return_value` / captures afterwards. The input and
    /// the construction parameters persist across calls.
    fn emit_scan_at(&self, out: &mut String) {
        out.push_str("    pub fn scan_at(&mut self, start: usize) -> bool {\n");
        out.push_str("        self.cursor = start;\n");
        ...
        for (f, _) in self.mode_c_inst_fields() {
            writeln!(out, "        self.{} = None;", f).ok();
        }
        out.push_str("        self.run();\n");
        out.push_str("        if self.accepted { self.reject_position = 0; }\n");
        out.push_str("        self.accepted\n    }\n\n");
    }
    ...
    /// flat `ops`/`rng` arrays (`fsm_regex::pike::encode`). Returns the
    /// leftmost-first match-end position from the cursor, or -1. `ops` is 4
    /// ints per instruction `[op, a, b, _]`: 0 Char (a = pair index, b = pair
    ...
    fn emit_pike_matcher(&self, out: &mut String) {
        ...
        writeln!(
            ...
            "    fn pike_is_word(&self, p: i64, word: &[i64]) -> bool {{\n\
             \x20       if p < 0 || p as usize >= self.{input}.fsm_len() {{ return false; }}\n\
             \x20       let v = self.{input}.fsm_get(p as usize) as i64;\n\
             \x20       let mut k = 0;\n\
             ...
             \x20           0 => pos == 0,\n\
             \x20           1 => pos == n,\n\
             \x20           2 => pos == 0 || self.{input}.fsm_get(pos - 1) as i64 == 10,\n\
             \x20           3 => pos == n || self.{input}.fsm_get(pos) as i64 == 10,\n\
             ...
             \x20       let mut pos = self.cursor;\n\
             \x20       loop {{\n\
             \x20           let mut nlist: Vec<usize> = Vec::new();\n\
             \x20           let mut nseen = vec![false; ninst];\n\
             \x20           for &pc in &clist {{\n\
             \x20               match ops[pc * 4] {{\n\
             \x20                   0 => {{\n\
             \x20                       if pos < n {{\n\
             \x20                           let v = self.{input}.fsm_get(pos) as i64;\n\
             ...
             \x20                       }}\n\
             \x20                   }}\n\
             \x20                   3 => {{ matched = pos as i64; break; }}\n\
             \x20                   _ => {{}}\n\
             \x20               }}\n\
             \x20           }}\n\
             \x20           if pos >= n {{ break; }}\n\
    ...
    fn emit_anchor_guards(&self, out: &mut String, sid: usize, ind: &str) {
        ...
        if dfa.requires_end {
            ...
            writeln!(
                out,
                "{ind}if _r != self.{input}.fsm_len() as i64 {{ _r = -1; }}"
            )
            .ok();
        }
    ...
    fn emit_word_boundary_helper(&self, out: &mut String) {
        ...
        writeln!(
            ...
            "    fn iswordat(&self, p: i64) -> bool {{\n\
             \x20       if p < 0 || p as usize >= self.{input}.fsm_len() {{ return false; }}\n\
             \x20       let b = self.{input}.fsm_get(p as usize);\n\
    ...
    fn emit_mode_c(
        ...
        let input = &self.decl.params[0].name;
        // Drive the inner over an OWNED window materialized through the outer's
        // accessor (RFC-0042.1): the inner is the owned form (`Inner<Vec<…>>`),
        // so storing it in `self` carries no borrow of `self.{input}`.
        writeln!(
            out,
            "{}let _inner = {}::new((self.cursor..self.{}.fsm_len()).map(|__k| self.{}.fsm_get(__k)).collect::<{}>());",
            ind, inner, input, input, self.input_type()
        )
        .ok();
        ...
        let slice = format!(
            "(self.cursor..(self.cursor + _icur)).map(|__k| self.{}.fsm_get(__k)).collect()",
            input
        );
    ...
    fn emit_dfa_const(&self, out: &mut String, sid: usize, ind: &str) {
        let dfa = &self.stage_dfas[sid];
        // A lazy stage emits its Pike program as two int-array consts.
    ...
    fn call(&self, func: &str, args: &[Expression]) -> String {
        ...
        match func {
            ...
            // The input field is the generic source `I`, indexed via the
            // `<Name>Input` trait — its element count is `fsm_len`, not `len`.
            "len" if a.len() == 1 && a[0] == format!("self.{}", self.decl.params[0].name) => {
                format!("(self.{}.fsm_len() as i64)", self.decl.params[0].name)
            }
...
/// Comma-joined `i64` list (the Pike `OPS`/`RNG` const bodies).
...
mod tests {
    ...
    /// Compile a full generated-code-plus-`main` program and return its stdout.
    /// `None` if `rustc` is unavailable.
    fn compile_run(program: &str, tag: &str) -> Option<String> {
        let dir = std::env::temp_dir();
        let src_path = dir.join(format!("framec_rs_{}.rs", tag));
        let bin_path = dir.join(format!("framec_rs_{}", tag));
        std::fs::write(&src_path, program).expect("write rs");
        let compile = match Command::new("rustc")
            .arg("-O")
            .arg("--edition=2021")
            .arg(&src_path)
    ...
    /// RFC-0042.1 — a recognizer built with `over` drives `scan_at` from many
    /// offsets over one input; the three input sources (owned `Vec<char>`,
    /// borrowed `&[u8]`, host callback `MFn`) agree, and each `scan_at` matches
    /// a fresh `new` over the tail (re-seed parity, no state leak).
    #[test]
    fn rust_scan_at_three_sources() {
        let src = "@@fsm M(src: bytes) : bool = false { /-?[0-9]+(\\.[0-9]+)?/ true }";
        let decl = parse_fsm_block(src.as_bytes()).expect("parses");
        let code = generate(&decl).expect("generates");
        let main = r#"
fn main() {
    // owned Vec<char> + scan_at reuse over "12 34 5"
    let mut a = M::over("12 34 5".chars().collect::<Vec<char>>());
    assert!(a.scan_at(0) && a.cursor == 2);
    assert!(a.scan_at(3) && a.cursor == 5);
    assert!(a.scan_at(6) && a.cursor == 7);
    assert!(!a.scan_at(2)); // a space is not a number
    // borrowed &[u8] — zero-copy over the host's bytes
    let bytes = b"7.5x";
    let mut b = M::over(&bytes[..]);
    assert!(b.scan_at(0) && b.cursor == 3);
    // host callback
    let s: Vec<char> = "42".chars().collect();
    let mut c = M::over(MFn(move |i| s[i], 2));
    assert!(c.scan_at(0) && c.cursor == 2);
    // re-seed parity: scan_at(start) over the whole == new() over the tail
    let whole: Vec<char> = "12 34 5".chars().collect();
    let mut p = M::over(whole.clone());
    for start in [0usize, 3, 6] {
        let acc = p.scan_at(start);
        let fresh = M::new(whole[start..].to_vec());
        assert_eq!(acc, fresh.accepted);
        assert_eq!(p.cursor - start, fresh.cursor);
        assert_eq!(p.return_value, fresh.return_value);
    }
    println!("ALL OK");
}
"#;
        let program = format!("{code}\n{main}");
        let Some(out) = compile_run(&program, "scan_at_three") else {
            return;
        };
        assert_eq!(out.trim(), "ALL OK", "scan_at / three-source driver failed");
    }

    /// RFC-0042.1 — `scan_at` resets domain state between calls: a domain field
    /// mutated during one scan does not leak into the next.
    #[test]
    fn rust_scan_at_resets_domain() {
        // `seen` counts matched digits via a per-element embedding action; each
        // scan must start from the initializer (0), not accumulate.
        let src = "@@fsm D(src: bytes) : int = 0 { /[0-9]+/ ${ self.seen = self.seen + 1 } self.seen domain: seen: int = 0 }";
        let decl = parse_fsm_block(src.as_bytes()).expect("parses");
        let code = generate(&decl).expect("generates");
        let main = r#"
fn main() {
    let mut d = D::over("123 45".chars().collect::<Vec<char>>());
    d.scan_at(0);
    let first = d.return_value;     // 3 digits
    d.scan_at(4);
    let second = d.return_value;    // 2 digits — NOT 3+2 if reset works
    println!("{} {}", first, second);
}
"#;
        let program = format!("{code}\n{main}");
        let Some(out) = compile_run(&program, "scan_at_reset") else {
            return;
        };
        assert_eq!(out.trim(), "3 2", "domain field leaked across scan_at");
    }
    ...
    fn rust_conditional_target() {
        ...
        let run_mode = |inp: &str, mode: i64, tag: &str| -> Option<String> {
            let driver = format!(
                "{code}\nfn main() {{ let m = M::new(\"{inp}\".chars().collect::<Vec<char>>(), {mode}); println!(\"{{}}\", m.return_value); }}\n",
    ...
    fn rust_mode_c_callout() {
        ...
        let run_outer = |inp: &str, tag: &str| -> Option<(String, String)> {
            let driver = format!(
                "{inner}\n{outer}\nfn main() {{ let m = Outer::new(\"{inp}\".chars().collect::<Vec<char>>()); println!(\"{{}}\", m.accepted); println!(\"{{}}\", m.return_value); }}\n",
    ...
    /// comma (cursor 3 over "ab,cd,ef"); mixed `/a*?b+/` keeps `b+` greedy
    /// (cursor 5 over "aabbb", not 3).
    #[test]
    fn rust_lazy_quantifier() {
        let first = "@@fsm M(text: bytes) : int = 0 { /.*?,/ @@:cursor }";
        let Some((_, r)) = run(first, "M", "ab,cd,ef", "lz_a") else {
            return;
        };
        assert_eq!(r, "3");
