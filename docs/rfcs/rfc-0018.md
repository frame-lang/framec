---
title: "RFC-0018: Re-entrant interface dispatch from lifecycle handlers"
nav_exclude: true
---
...
> **Reader's note.** This RFC was written before RFC-0019 removed the
> enter / exit cascade. Lifecycle-semantics passages below quote the
> pre-RFC-0019 contract (kernel walks the ancestor chain on `$>` / `<$`).
> The construction context-push fix it documents survives RFC-0019
> unchanged. For the current `$>` / `<$` dispatch contract — leaf-only,
> ancestors via explicit `=> $^` forward — see [RFC-0019](rfc-0019.md).
