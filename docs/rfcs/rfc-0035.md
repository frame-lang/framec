...
- **Round 13 (judgement call) — the `pipeline_parser` `parse_*`
  oracle methods.** Native by RFC-0039's backbone+oracle design;
  under the strict mandate they're also state machines in Rust.
  Convert only if a round demonstrates net clarity, not ceremony.
