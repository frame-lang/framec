...
- **Round 3**: Erlang line classifier as a multi-state Frame
  system. Will probably surface more ergonomic gaps — line
  classifiers are slightly outside Frame's natural fit, and
  documenting where they chafe is the point.
- **Round 4**: Pick 1–2 validator passes (single-pass AST
  walkers with classification logic) and re-implement as
  multi-state Frame FSMs.
...
| Erlang per-line native rewriter | **Round 3** — planned |
