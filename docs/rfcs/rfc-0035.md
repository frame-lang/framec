...
The migration backlog is now exhausted of immediately-planned
items. Future rounds will be opportunistic:

- **`validate_reachable_states`** (BFS over transition targets)
  — another graph algorithm; could benefit from the same
  shape as Round 5. The visited set + queue would both be
  domain fields. Deferred until a use case requires touching
  the function.
