...
Future rounds will be opportunistic — driven by what's being
touched. Candidates whose shape might still teach something
new:

- **Cross-target attribute scanners** — if a new attribute
  family lands, the scanner naturally fits the existing
  `attribute_scanner.frs` template.
- **Codegen backend dispatch** — `LanguageBackend::emit` is a
  per-target dispatcher. The dispatch itself could be modeled
  as an FSM (`$Selecting → $Emitting<lang>`), though the
  value-add is probably ceremony rather than a real demo.
- **Per-target attribute scanners** beyond the existing
  `attribute_scanner.frs` — if a new attribute family adds
  another scanner, that's a natural FSM candidate.
- **Pipeline orchestration** — currently the pipeline is
  hand-coded Rust. Some sub-phase (e.g. the prolog scanner)
  could become a Frame system to demonstrate the language on
  a meta-compiler shape.
...
| Pipeline supervisor (meta-compiler observer FSM) | **Done — Round 7.** `compiler/pipeline_supervisor/` |
