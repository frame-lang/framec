...
- **Larger validator passes** (`validate_hsm_cycles`,
  `validate_reachable_states`) — both are graph algorithms
  with clearly multi-state semantics. Whether they migrate
  depends on whether the graph-input serialization cost
  (passing `HashMap<String, &StateAst>` through a Frame
  interface) is worth the dogfood demonstration.
