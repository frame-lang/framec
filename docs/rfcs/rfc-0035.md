...
The migration backlog has now covered: name converters (R1),
type mappers + dispatch helpers (R2), line classifiers (R3),
stateful validators with terminal error states (R4), and graph
algorithms in two flavors (R5 per-chain FSM, R6 whole-walk
FSM). The RFC has its "diverse problems Frame CAN be applied
to" inventory.

Future rounds will be opportunistic — driven by what's being
touched, not by chasing the backlog. Candidates whose shape
might still teach something new:

- **Pipeline orchestration** — the multi-phase compile
  pipeline (segment → parse → validate → codegen → assemble)
  is itself a state machine. Migrating the orchestrator would
  put Frame on the meta-compiler.
