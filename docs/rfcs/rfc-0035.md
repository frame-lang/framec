...
The supervisor is **observational** — it does not drive the
orchestrator's control flow. The orchestrator in
`pipeline/compiler.rs` stays in native Rust and calls into the
supervisor at each phase boundary (`begin_phase("segment")`,
`abort("E001", ...)`, etc.). Wrong supervisor logic produces
wrong `--debug` output but cannot change compilation
correctness. That bounded-risk profile is what made Round 7
tractable: the orchestrator is 1500 lines of intertwined
linear logic with many bookkeeping side effects; rewriting it
as an FSM-DRIVEN controller would be a multi-day arc with high
regression risk. The supervisor-as-observer shape gives the
dogfood demo without rewriting the pipeline.
...
- **Round 8 — make the pipeline supervisor *drive*.** R7 shipped
  it **observational** (it mirrors the orchestrator but does not
  control it — see the note under Round 7). `pipeline/compiler.rs`
  (`compile_ast_based`, ~1560 lines) has genuine control structure
  — per-phase error→abort, the graphviz-vs-codegen fork, target
  dispatch, a system loop — that an FSM should *own* as states +
  transitions, holding the intermediate pipeline state in domain
  fields (the RFC-0039 `B1` "backbone owns the state" pattern).
  Observational is the worst middle; this closes it.
- **Round 9 — `domain.rs` outer scanner.** The `domain:` section's
  line-structure walk (~250 LOC of native byte-stepping:
  blank-skip, indent, section-end detect, comment, field) is still
  hand-rolled; only its `@@[…]` and default-expression sub-parts
  delegate to FSMs. A multi-state line scanner — a natural `.frs`.
- **Round 10 — `assembler/mod.rs` call-site scanner** (~1344 LOC;
  `match c` byte scan that finds and rewrites `@@SystemName(...)`
  instantiations in native code).
- **Round 11 — `native_region_scanner/unified/metadata.rs`** (369
  LOC; a native "scanner parsing phase," `match bytes[p]`, living
  *inside* the FSM-scanner directory while itself native).
...
| Pipeline supervisor (meta-compiler observer FSM) | **Observational only — Round 7.** `compiler/pipeline_supervisor/`. It mirrors the pipeline but does NOT drive it; **Round 8 (open)** makes it actually control `compile_ast_based`. |
