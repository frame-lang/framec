...
- **Status**: Draft
- **Author**: Mark Truluck
- **Amended by**: [RFC-0042.1](rfc-0042-1.md) (pluggable input source — a
  recognizer generic over an owned buffer / borrowed slice / host callback, with
  a reusable `over`/`scan_at` drive form for scan-at-cursor use; narrows the
  deferred §11.3).
...
- **Companion**: [RFC-0020](rfc-0020.md) (runtime reference architecture for `@@system`; `@@fsm` has its own runtime model and is not bound by RFC-0020's compartment shape); [RFC-0050](rfc-0050.md) (Frame statement syntax — the grammar `@@fsm` action bodies use).
- **Depends on**: [RFC-0050](rfc-0050.md) — must ship before `@@fsm` action bodies can be parsed.
...
`@@fsm` action bodies admit Frame statement-level code as defined by [RFC-0050](rfc-0050.md). That RFC is the canonical reference for the statement grammar (assignment, call, `if/else`, return-value), expression syntax (literals, references, operators, precedence), lvalue rules, scope rules, and Frame-level comment forms (`//`, `/* */`).
...
> persisted DFA-state) remains deferred here. The simpler **single-shot scan
> over a fully-present, host-owned source** (an owned buffer, a borrowed slice,
> or a host callback) via `over`/`scan_at` — recognition starting at a host
> cursor with no suspended state, the lexer-primitive case — is specified by
> RFC-0042.1 and is *not* part of this deferred item.
...
**Context:** An empty regex has no clear semantic. However, the spelling `//`
is **lexically a line comment** inside `@@fsm` (§3.5 / RFC-0043), so an empty
regex **cannot be written in source** — the case below is a comment that
swallows the closing `}`:

```frame
@@fsm M(text: bytes) : bool = false { // true }
```

**Expected outcome:** `E001` (unterminated `@@fsm` block) whose message notes
that `//` begins a line comment and an empty regex cannot be written as `//`.
**E723 is engine-internal**: it guards the regex compile API (defense in
depth, unit-tested in `fsm_regex`) and is unreachable from source by
construction.
