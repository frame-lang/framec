...
| Anchors `^ $ \A \z` | partial | leading/trailing only; an interior anchor is E722 (§6.6) |
| Word boundaries `\b \B` | partial | `bytes` alphabet, pattern edges only (§6.6); interior, and `char`/`token`, deferred |
| Unicode classes `\p{...} \P{...}` | partial | `char` alphabet, opt-in `@@[allow(unicode_classes)]` (§6.7/§11.6) |
| Shorthands `\d \w \s` | partial | ASCII definitions; Unicode-aware forms deferred (§11.6) |
| Inline flags `(?i) (?m) (?s)`, case-insensitive | deferred | unsupported; `@@[multiline]` is the only mode attribute, and only at edge anchors |
...
**Remaining to reach feature parity** — all incremental on the existing engine, no new runtime architecture: interior anchors and word boundaries; `\b`/`\p{}`/Unicode shorthands on the `char` alphabet (a shared Unicode-classification effort); inline flags and case-insensitive matching; multiline `^`/`$` line semantics.
...
classes (§6.7/§11.6), resolved to codepoint ranges at compile time, and lazy
quantifiers (`*?`/`+?`/…, §11.1) on bytes/char via a per-stage Pike VM for
leftmost-first match-end semantics (greedy stays pure-DFA).
...
need a Unicode word classification — §11.6), lazy quantifiers on the `token`
alphabet or in a stage with embedding actions (§11.1), a Mode C stage used as a
`|` alternative selector, and a `|` alternative with elements preceding its
first stage. These are the remaining deferrals of §6.5 and the composition
restrictions of §8, surfaced at compile time. (Edge `^`/`$`/`\A`/`\z` and, on
`bytes`, edge `\b`/`\B` *are* supported — extracted to position requirements
