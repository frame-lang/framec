...
Mode C call-out (§8.3), the edge anchors of §6.6 — leading/trailing
`^`/`$`/`\A`/`\z` plus, on the `bytes` alphabet, leading/trailing `\b`/`\B`
word boundaries (enforced by the matcher against the live cursor) — and, on the
`char` alphabet behind `@@[allow(unicode_classes)]`, `\p{...}`/`\P{...}` Unicode
classes (§6.7/§11.6), resolved to codepoint ranges at compile time.
