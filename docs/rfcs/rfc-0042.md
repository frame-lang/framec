...
When all elements succeed, the match follows its `success_branch` transition. Recognition terminates with `accepted = true` when execution reaches a terminal state (one with no pending match elements and no transition out).

The `accepted` field reflects only whether recognition completed normally — that is, whether execution reached a terminal state. **State names have no semantic meaning to the framepiler.** A state the writer names `$error`, `$reject`, or `$fail` is still just a terminal state; reaching it via a `failure_branch` produces `accepted = true` because recognition completed normally. The `return_value` is whatever that state's match assigned. To distinguish "recognition succeeded" from "recognition routed to a user-defined error path," the writer relies on `return_value` (typically by encoding the distinction in the return type, e.g., with a `Result`-shaped sum or a sentinel value). `accepted = false` occurs only when a stage fails and no `failure_branch` is available to route the failure (§5.6).
...
**Context:** Tests the explicit `failure_branch` form: success and failure both name target states. Demonstrates the rule pinned in §5.3 — `accepted` reflects whether recognition completed normally, not whether the writer-named state was conceptually "success" or "failure." A state named `$error` is still a terminal state; reaching it via the failure branch yields `accepted == true` with the user's chosen `return_value`.
...
**Behavior:**
- `Assert(@@M("foo").accepted == true)` — `/foo/` matches; transitions to `$accept`; terminates normally.
- `Assert(@@M("foo").return_value == true)`.
- `Assert(@@M("bar").accepted == true)` — `/foo/` fails; failure_branch routes to `$error`; terminates normally. Recognition reached a terminal state, so `accepted` is true.
- `Assert(@@M("bar").return_value == false)` — the `$error` state's bare expression assigned `false` to `@@:return`.

**What this test verifies:**
- The `: -> $error` syntax parses correctly.
- Failure routing transfers control to the named state.
- `accepted` is `true` whenever recognition reaches any terminal state, regardless of the state's name or the writer's intent for it.
