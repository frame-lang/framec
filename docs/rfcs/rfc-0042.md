...
Mode C call-out (§8.3), and the boundary anchors of §6.6.
...
a clear, uniform error (never a silent miscompile): mid-pattern anchors and
`\b`/`\B` word boundaries, a Mode C stage used as a `|` alternative selector,
and a `|` alternative with elements preceding its first stage. These are the
v0.1 deferrals of §6.5 and the composition restrictions of §8, surfaced at
compile time.
