...
**Acceptance is decided by the *terminating event*, not by the state's name and not by whether a failure occurred earlier en route.** Recognition halts with `accepted = true` when the terminating event is a *successful match completion* at a terminal state — reached via a `success_branch`, or as an implicit-terminal match whose stages all matched. It halts with `accepted = false` when the terminating event is a *stage failure* that cannot make further progress: the match's `failure_branch` routes to a terminal state, or no `failure_branch` exists (§5.6).
