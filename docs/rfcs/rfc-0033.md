...
idiomatic Rust developer would write by hand: accept borrowed parameter
types (`&str`, `&[T]`) in interface signatures, emit a module-level
lint-suppression preamble matching the convention of established Rust
code generators, and let state-variable declarations use full
expression-form initializers like `String::from("default")` or
`Vec::with_capacity(8)`.
...
2. **Lint-deny CI gates.** The standard Rust quality gate is
   ...
   (bindgen, prost, tonic-build, pest) emit a fixed inner-attribute
   preamble at the top of each generated module so downstream crates
   can keep that gate on without false positives. Frame's Rust output
   lacks this preamble, plus it triggers three specific clippy lints
   (`clippy::new_without_default`, `clippy::needless_return`, and
   `unused_variables`) that compound the noise.
...
### Module-level lint-suppression preamble

Every Rust file framec emits **MUST** begin with the following inner-
attribute preamble, before any `use` statements:
...
This matches the convention established by bindgen, prost, tonic-build,
and pest. It lets downstream crates run `cargo clippy -D warnings`
without false positives on Frame-emitted code.

The per-item `#[allow(dead_code)]` annotations framec currently emits
**MAY** remain — they do not conflict with the module-level umbrella
and stay useful for tools that consume framec output piecewise.

### `Default` impl for the system struct

Every system struct framec emits **MUST** be accompanied by an
`impl Default` that delegates to `new`:

```rust
impl Default for FooSystem {
    fn default() -> Self {
        Self::new()
    }
}
...
This closes the `clippy::new_without_default` lint without forcing the
user to write the boilerplate themselves.

### No trailing `return;` after transitions

In dispatch match arms whose last statement is a
`self.__transition(...)` call, framec **MUST NOT** emit a trailing
`return;`. Rust falls through to the end of the arm; the explicit
return is redundant and triggers `clippy::needless_return`.
...
file carries its own `#![allow(...)]` preamble, the `clippy::new_without_default`
lint is satisfied by the auto-generated `impl Default`, and
`clippy::needless_return` doesn't fire because trailing `return;` after
transitions is gone.
...
- `Default` impl: adds a trait implementation; existing call sites are
  unaffected.
- Trailing `return;` elision: removes a redundant statement; behavior
  unchanged.
...
- RFC-0019 — Runtime layout that motivates the owned-event-payload
  invariant.
- RFC-0025 — Quality-remediation pass that this RFC's hygiene
  changes (lint preamble, `Default` impl, `return;` elision) are
  a continuation of.
