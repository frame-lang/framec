---
title: "RFC-0020 — Runtime Reference Architecture"
nav_exclude: true
---
...
> **Aligned with RFC-0019 (2026-05-12).** RFC-0019 removed the enter / exit
> cascade after RFC-0020's first cut. The Compartment and FrameContext
> shapes documented below are unchanged; what changed is the kernel's
> behavior on `$>` / `<$` — both now leaf-dispatch only. Per-state
> ancestor lifecycle runs *only* via explicit `=> $^` in the leaf's
> handler body, not via a runtime cascade. Anywhere this RFC discusses
> "the kernel walks the ancestor chain and fires `$>` / `<$`" should be
> read as describing the pre-RFC-0019 contract; the current contract is
> "leaf-only, forwarded via `=> $^` when the user opts in." See
> [RFC-0019](rfc-0019.md) for the full new contract.

> **Scope clarification — `@@system` only.** This RFC is the normative
> runtime reference for **`@@system`**. Frame's other constructs are not
> bound by the compartment / context-stack model documented here:
>
> - [`@@fsm`](rfc-0042.md) (RFC-0042, Draft) — finite-state recognizer
>   construct. Has its own runtime model: no compartments, no
>   lifecycle handlers, runs to completion at construction.
> - `@@pda` (future) — context-free recognizer construct, to be specified.
>
> Per-construct target codegen divergence (separate emission strategies
> for `@@system`, `@@fsm`, and `@@pda`) is anticipated as those
> constructs land. This RFC remains the authoritative reference for
> `@@system`'s runtime; companion RFCs will define the runtime for each
> non-`@@system` construct as it is introduced.
