...
- **Child-system calls.** Composing another system (a `@@<Other>()` instance held in a state variable or domain field) is native code: the transpiler lowers the `$.sensor` / `self.sensor` **reference** and the `@@Sensor()` **construction**, but the **method call is user-authored** — write `$.sensor:bump()` on Lua, `$.sensor.bump()` on Python, `module:bump($.sensor)` on Erlang, `Counter_bump($.sensor)` on C. The codegen does not alloc, free, or call the child for you; that is your code, in the target's native form.
