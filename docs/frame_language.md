...
Only **interface methods** dispatch through the kernel. Actions and operations are *native*: the transpiler lowers the Frame **reference** but passes the **call syntax through unchanged** (Oceans Model) — it does **not** rewrite `.`/`:` or the receiver spelling.
...
- **Child-system calls (embedded-system calls).** Composing another system — a `@@<Other>()` instance held in a **domain field whose declared type is a defined system** — is Frame's own syntax, and framec **lowers the whole call** (RFC-0046). Write `@@:self.<field>.<method>(args)`; framec emits it in the target's idiom, routed through the child's generated interface: `self.sensor.bump(args)` on Python/Rust/Java/most targets (`.`/`->` chosen by framec, never by you), and the cross-system free-function form `Sensor_bump(self->sensor, args)` on C (and the equivalent process/module dispatch on Erlang). Because the whole chain is framec-emitted, framec also owns the storage representation, so no `.`-vs-`->` hint is needed. framec still does **not** alloc or free the child for you — construct it with `@@Sensor()` and (on manual-memory targets) free it yourself. *(The distinction is read from the field's declared type: a field whose type is a defined system is an embed; anything else is a scalar, and `@@:self.<field>.<method>(args)` on it is an ordinary native method call on the field's value.)*
