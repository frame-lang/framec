...
//
// A LINEAR 4-STATE SPINE (the decl_read mode-spine shape), NOT a cycle. Each phase is one state
...
//   $Interface -> emit_interface::walk (the `(method, arm)` router pass)   -> $Handlers
...
//
// framec owns the SPINE (the four unconditional advances + the persist guard). The un-Frame-able
// work is per-phase NATIVE LEAVES: `emit_iface_phase` / `emit_handlers_phase` / `emit_actions_phase`
// each call ONE already-landed sub-system's `walk` (unchanged, NOT reinlined); `manifest_enabled`
// reads the persist flag; and `emit_persist` spells the one `be.persist(...)` the hand pass ran.
//
